    ...
    try {
        ...
        switch (format) {
            ...
            case "takeaways":
                prompt = `Extract ${
                    length * 2
                } key takeaways from the following text:\n\n${text}`;
                break;
            case "brief":
            default:
...
Comments: ${thread.comments ? thread.comments.join("\n") : ""}
    `;
        ...
        switch (format) {
            case "bullets":
                prompt = `Summarize the following thread in ${
                    length * 2
                } bullet points, capturing both the main post and key points from comments:\n\n${threadText}`;
                break;
            case "takeaways":
                prompt = `Extract ${
                    length * 2
                } key takeaways from the following thread, including insights from both the main post and comments:\n\n${threadText}`;
                break;
            case "brief":
            default:
