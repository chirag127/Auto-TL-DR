        ...
        (loadedSettings) => {
            ...
            isThreadPage =
                ...
                document.querySelectorAll(
                    ".comment, .comments, .thread, .discussion"
                ).length > 5;
...
function addManualSummaryTrigger(element, type) {
    ...
    <span class="auto-tldr-trigger-text">Summarize this ${
...
function summarizeElement(element) {
    ...
    <div class="auto-tldr-header">
      ...
      <div class="auto-tldr-actions">
        <button class="auto-tldr-speak" title="Read summary aloud">🔊</button>
  ...
  `;
    ...
    summaryContainer
        .querySelector(".auto-tldr-regenerate")
        .addEventListener("click", () => {
    ...
    summaryContainer
        .querySelector(".auto-tldr-speak")
        .addEventListener("click", () => {
            speakSummary(summaryId);
        });
...
function extractThreadData() {
    ...
    if (isRedditPage) {
        ...
        const postContent = document.querySelector(
            ".Post__content, .ThreadViewer__content, .selftext"
        );
        if (postContent) {
        ...
        const commentElements = document.querySelectorAll(
            ".Comment__body, .ThreadComment__content"
        );
        ...
        // Extract post content
        const postContent = document.querySelector(
            ".post-content, .thread-content, .original-post"
        );
        if (postContent) {
            content = postContent.textContent.trim();
        }
...
function requestThreadSummary(summaryId, threadData) {
    ...
    fetch(`${apiUrl}/summarize/thread`, {
        ...
        .catch((error) => {
            ...
            updateSummaryContainer(
                summaryId,
                "Failed to generate thread summary. Please try again."
            );
        })
        .finally(() => {
            // Clear request in progress
...
function regenerateSummary(summaryId, element) {
    ...
    if (contentElement) {
        contentElement.innerHTML =
            '<div class="auto-tldr-loading">Regenerating summary...</div>';
    }
...
function regenerateThreadSummary(summaryId) {
    ...
    if (contentElement) {
        contentElement.innerHTML =
            '<div class="auto-tldr-loading">Regenerating thread summary...</div>';
    }
...
function summarizePage() {
    ...
    if (!summaryContainer) {
        ...
        summaryContainer.className =
            "auto-tldr-container auto-tldr-page-summary";
        summaryContainer.id = summaryId;
        summaryContainer.innerHTML = `
      <div class="auto-tldr-header">
        <div class="auto-tldr-logo">
    ...
    `;
        ...
        const contentElement =
            summaryContainer.querySelector(".auto-tldr-content");
        if (contentElement) {
            contentElement.innerHTML =
                '<div class="auto-tldr-loading">Generating page summary...</div>';
        }
...
function summarizeSelection() {
    ...
    if (!summaryContainer) {
        summaryContainer = document.createElement("div");
        summaryContainer.className =
            "auto-tldr-container auto-tldr-selection-summary";
        summaryContainer.id = summaryId;
        summaryContainer.innerHTML = `
      <div class="auto-tldr-header">
        <div class="auto-tldr-logo">
...
function speakSummary(summaryId) {
    ...
    const summaryElement = summaryContainer.querySelector(".auto-tldr-summary");
    if (!summaryElement) return;

    // Store the current speaking element
    currentSpeakingElement = summaryElement;

    // Get the summary text
    const text = summaryElement.textContent.trim();
    if (!text) return;

    // Prepare for word highlighting
    prepareWordHighlighting(summaryElement, text);
    ...
    if (settings.ttsVoice && window.speechSynthesis) {
    ...
    // Add event listeners for word highlighting
    ...
    // Update the speak button to show it's active
    const speakButton = summaryContainer.querySelector(".auto-tldr-speak");
    if (speakButton) {
        speakButton.innerHTML = "⏹️";
        speakButton.title = "Stop reading";

        // Change the click event to stop speech
        speakButton.onclick = stopSpeech;
    }
}

// Prepare the summary text for word highlighting
function prepareWordHighlighting(element, text) {
    ...
    words.forEach((word, index) => {
        html += `<span class="auto-tldr-word" data-index="${index}">${word}</span> `;
    });
    ...
    wordElements = Array.from(element.querySelectorAll(".auto-tldr-word"));
}
...
function handleWordBoundary(event) {
    ...
    wordElements.forEach((wordElement) => {
        wordElement.classList.remove("auto-tldr-word-highlight");
    });

    // Calculate the current word index
    const wordIndex = Math.floor(event.charIndex / 5); // Approximate word index

    // Highlight the current word if it exists
    if (wordIndex >= 0 && wordIndex < wordElements.length) {
        wordElements[wordIndex].classList.add("auto-tldr-word-highlight");
...
function handleSpeechEnd() {
    ...
    // Reset the speak button
    if (currentSpeakingElement) {
        const summaryContainer = currentSpeakingElement.closest(
            ".auto-tldr-container"
        );
        if (summaryContainer) {
            const speakButton =
                summaryContainer.querySelector(".auto-tldr-speak");
            if (speakButton) {
                speakButton.innerHTML = "🔊";
                speakButton.title = "Read summary aloud";

                // Reset the click event
                const summaryId = summaryContainer.id;
                speakButton.onclick = () => speakSummary(summaryId);
            }
        }
    }

    // Clear current utterance and element
...
function stopSpeech() {
    ...
    // Reset the speak button if there's an active one
    if (currentSpeakingElement) {
        const summaryContainer = currentSpeakingElement.closest(
            ".auto-tldr-container"
        );
